    ...
    setScore((prevScore: any) => {
      ...
      if (amount > 0) {
        if ((newScore >= pointsToWin && newScore - opponentScore >= 2) || newScore === maxPoints) {
          ...
          // Move to next game automatically (always play all games, even if match is already won)
          ...
          if (nextGame < newGames.length) {
            setTimeout(() => {
              ...
              if (!newGames[nextGame]?.type) {
                ...
                toast({
                  title: `✅ Game ${currentGame + 1} Complete!`,
                  description: `${team === "home" ? match.homeTeam.name : match.awayTeam.name} wins ${newScore}-${opponentScore}. Moving to Game ${nextGame + 1}.`,
                })
              }
            }, 500)
          } else {
            // All games completed - Check if match is won and complete
    ...
    return (
      <Card className="shadow-lg border-t-4 border-t-yellow-600">
        <CardHeader className="bg-gradient-to-r from-purple-500 to-violet-600 text-white rounded-t-lg">
          <CardTitle className="text-center text-xl md:text-2xl flex items-center justify-center gap-2">🏸 Badminton Scoreboard</CardTitle>
        </CardHeader>
